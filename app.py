    ...
    try:
        ...
        stats = db.execute_query('''
            SELECT
                ...
                COALESCE(AVG(CASE WHEN status != 'cancelled' THEN total_amount ELSE NULL END), 0) as avg_order
            FROM orders
            WHERE user_id = ?
        ''', (customer_id,))
...
def crm_page():
    try:
        ...
        rows = db.execute_query("""
            SELECT u.id, u.name, COUNT(o.id) as orders, COALESCE(SUM(o.total_amount),0) as spent,
    ...
    try:
        ...
        if target_audience == 'all':
            ...
            recipients = db.execute_query('''
                ...
                WHERE u.is_admin = 0 AND o.created_at >= ?
...
def chart_data():
    ...
    _raw = request.args.get('period', None)
    try:
        period = int(_raw) if _raw not in (None, "", "null", "None") else 30
...
def export_analytics():
    try:
        ...
        start_dt = (datetime.now() - timedelta(days=int(period))).strftime('%Y-%m-%d %H:%M:%S')
    ...
    try:
        ...
        financial_data = db.execute_query('''
            SELECT c.name as category,
                   COALESCE(SUM(oi.quantity * COALESCE(oi.price, 0)), 0) as revenue,
                   COALESCE(SUM(oi.quantity * COALESCE(p.cost_price, 0)), 0) as cost,
                   COALESCE(SUM(oi.quantity * COALESCE(oi.price, 0)) - SUM(oi.quantity * COALESCE(p.cost_price, 0)), 0) as profit,
                   COUNT(DISTINCT o.id) as orders
            FROM categories c
...
def profit_report():
    ...
    try:
        ...
        names = []
        try:
            cols = db.execute_query("SELECT column_name FROM information_schema.columns WHERE table_name = 'order_items'") or []
    ...
    if price_col:
        product_rows = db.execute_query(f'''
            SELECT p.id, p.name,
                   COALESCE(SUM(oi.quantity * oi.{price_col}), 0) as revenue,
                   COALESCE(SUM(oi.quantity * COALESCE(p.cost_price,0)), 0) as cost,
                   COALESCE(SUM(oi.quantity * oi.{price_col}), 0) - COALESCE(SUM(oi.quantity * COALESCE(p.cost_price,0)), 0) as profit
            FROM products p
            LEFT JOIN order_items oi ON oi.product_id = p.id
            GROUP BY p.id, p.name
        ...
        product_rows = db.execute_query('''
            SELECT p.id, p.name,
                   0 as revenue,
                   COALESCE(SUM(oi.quantity * COALESCE(p.cost_price,0)), 0) as cost,
                   0 - COALESCE(SUM(oi.quantity * COALESCE(p.cost_price,0)), 0) as profit
            FROM products p
            LEFT JOIN order_items oi ON oi.product_id = p.id
            GROUP BY p.id, p.name
            ORDER BY profit DESC
...
def crm_quick_action():
    ...
    try:
        if action == 'send_coupon':
            ...
            db.execute_query('INSERT INTO promo_codes (code, discount_type, discount_value, is_active, created_at) VALUES (?, ?, ?, 1, ?)', (code, 'percent', 10, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
            # try notify user
...
def inventory_page():
    try:
        rows = db.execute_query('''
            SELECT COUNT(*), COALESCE(SUM(stock),0), COALESCE(SUM(stock*COALESCE(cost_price,0)),0)
...
@login_required
def financial_page():
    sales = db.execute_query("SELECT COALESCE(SUM(total_amount),0), COUNT(*), COALESCE(AVG(total_amount),0) FROM orders WHERE status!='cancelled'") or [(0,0,0)]
    revenue, orders_count, aov = sales[0]
    cost_row = db.execute_query("SELECT COALESCE(SUM(oi.quantity * COALESCE(p.cost_price,0)),0) FROM order_items oi JOIN products p ON p.id=oi.product_id JOIN orders o ON o.id=oi.order_id WHERE o.status!='cancelled'") or [(0,)]
    cost = cost_row[0][0] if isinstance(cost_row[0], (list, tuple)) else cost_row[0]
    profit = (revenue or 0) - (cost or 0)
    by_cat = db.execute_query("SELECT c.name, COALESCE(SUM(oi.quantity*COALESCE(oi.price,0)),0) as rev FROM categories c LEFT JOIN products p ON p.category_id=c.id LEFT JOIN order_items oi ON oi.product_id=p.id LEFT JOIN orders o ON o.id=oi.order_id AND o.status!='cancelled' GROUP BY c.id, c.name ORDER BY rev DESC LIMIT 10") or []
